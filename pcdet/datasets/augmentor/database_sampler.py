...
from ...utils import box_utils, box2d_utils, calibration_kitti
from pcdet.datasets.kitti.kitti_object_eval_python import kitti_common

class DataBaseSamplerCMAug(object):

    def __init__(self, root_path, sampler_cfg, class_names, logger=None):
        self.root_path = root_path
        self.class_names = class_names
        self.sampler_cfg = sampler_cfg
        self.aug_with_img = sampler_cfg.get('AUG_WITH_IMAGE', True)
        self.joint_sample = sampler_cfg.get('JOINT_SAMPLE', True)
        self.keep_raw = sampler_cfg.get('KEEP_RAW', True)
        self.box_iou_thres = sampler_cfg.get('BOX_IOU_THRES', 0.5)
        self.aug_use_type = sampler_cfg.get('AUG_USE_TYPE', 'annotation')
        self.point_refine = sampler_cfg.get('POINT_REFINE', True)
    ...
    def filter_by_min_pixels(self, db_infos, min_pixels):
        new_db_infos = {}
        for key, dinfos in db_infos.items():
            pre_len = len(dinfos)
            new_db_infos[key] = [
                info for info in dinfos
                if (info['bbox'][3]-info['bbox'][1])*(info['bbox'][2]-info['bbox'][0]) > min_pixels
            ]
            if self.logger is not None:
                self.logger.info('Database filter by min pixels %s: %d => %d' % (key, pre_len, len(new_db_infos[key])))
        return new_db_infos
    ...
    def sample_with_fixed_number(self, class_name, sample_group):
        ...
        sampled_dict = [self.db_infos[class_name][idx] for idx in
                        indices[pointer: pointer + sample_num]]
        pointer += sample_num
    ...
    def copy_paste_to_image(self, data_dict, crop_feat, gt_number, point_idxes=None):
        image, boxes3d, boxes2d = data_dict['images'], data_dict['gt_boxes'], data_dict['gt_boxes2d']
    ...
    def add_sampled_boxes_to_scene(self, data_dict, sampled_gt_boxes, mv_height, sampled_gt_boxes2d,
                                   total_valid_sampled_dict):
        gt_boxes_mask = data_dict['gt_boxes_mask']
        gt_boxes, gt_names, points = data_dict['gt_boxes'][gt_boxes_mask], data_dict['gt_names'][gt_boxes_mask], \
                                     data_dict['points']
        ...
        for idx, info in enumerate(total_valid_sampled_dict):
            ...
            if self.point_refine:
                ...
                points_rect = data_dict['calib'].img_to_rect(points_2d[:, 0], points_2d[:, 1], depth_2d)
                points_lidar = data_dict['calib'].rect_to_lidar(
                    points_rect)
            ...
            if self.aug_with_img:
                self.sampler_cfg.IMG_ROOT_PATH = 'training/image_2'
        ...
        data_dict['gt_boxes'], data_dict['gt_names'], data_dict['points'] = gt_boxes, gt_names, points
        if self.aug_with_img:
    ...
    def __call__(self, data_dict):
        gt_boxes, gt_names, existed_boxes = data_dict['gt_boxes'], data_dict['gt_names'].astype(str), data_dict['gt_boxes']
        ...
        data_dict['gt_boxes2d'] = existed_boxes2d

        total_valid_sampled_dict, sampled_gt_boxes = [], []
        sampled_gt_boxes2d = []
        for class_name, sample_group in self.sample_groups.items():
            ...
            if int(sample_group['sample_num']) > 0:
                ...
                if self.aug_with_img:
                    ...
                    sampled_boxes2d = torch.Tensor(sampled_boxes2d); existed_boxes2d = torch.Tensor(existed_boxes2d)
                    if self.use_oais:
                    ...
                    valid_mask = ((iou2d1.max(axis=1) < self.box_iou_thres) & (iou2d2.max(axis=1) < self.box_iou_thres) & ((iou1.max(axis=1) + iou2.max(axis=1)) == 0)).nonzero()[0]
                ...
                existed_boxes2d = np.concatenate((existed_boxes2d, sampled_boxes2d), axis=0)
        ...
        if total_valid_sampled_dict.__len__() > 0:
            data_dict = self.add_sampled_boxes_to_scene(data_dict, sampled_gt_boxes, None, sampled_gt_boxes2d,total_valid_sampled_dict)
        ...
        if 'points_2d' in data_dict:
            data_dict.pop('points_2d')
        if 'points_pasted' in data_dict:
            data_dict.pop('points_pasted')
        if 'points_no_pasted' in data_dict:
            data_dict.pop('points_no_pasted')
        return data_dict