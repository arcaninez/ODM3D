...
class DataAugmentor(object):
    ...
    def gt_sampling(self, config=None):
        db_sampler = database_sampler.DataBaseSamplerCMAug(
    ...
    def random_local_frustum_dropout(self, data_dict=None, config=None): # PPBA paper?
        """
        Please check the correctness of it before using.
        """
        if data_dict is None:
    ...
    def random_local_pyramid_aug(self, data_dict=None, config=None):
        ...
        gt_boxes, points, pyramids = augmentor_utils.local_pyramid_sparsify(gt_boxes, points, config['SPARSIFY_PROB'], config['SPARSIFY_MAX_NUM'], pyramids)
        gt_boxes, points = augmentor_utils.local_pyramid_swap(gt_boxes, points, config['SWAP_PROB'], config['SWAP_MAX_NUM'], pyramids)
        data_dict['gt_boxes'] = gt_boxes
        data_dict['points'] = points
        return data_dict
    ...
    def random_image_flip(self, data_dict=None, config=None):
        ...
        # imgs, gt_boxes, calib are required
        ...
        for cur_axis in config['ALONG_AXIS_LIST']:
            ...
            images, depth_maps, gt_boxes, gt_boxes2d = getattr(augmentor_utils, 'random_image_flip_%s' % cur_axis)(
                images, depth_maps, gt_boxes, calib, gt_boxes2d, W
            )
