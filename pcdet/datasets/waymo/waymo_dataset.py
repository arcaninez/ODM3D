...
# produce sync lidar points for each image, yu hong
    def get_sync_lidar(self, raw_data_path, save_path, num_workers=multiprocessing.cpu_count(), has_label=True, sampled_interval=1):
        from functools import partial
        print('---------------The waymo sample interval is %d, total sequecnes is %d-----------------'
              % (sampled_interval, len(self.sample_sequence_list)))

        produce_sync_lidar_files = partial(
            waymo_utils.produce_sync_lidar_files, # this function is not currently provided!
            save_path=save_path, sampled_interval=sampled_interval,
        )
        sample_sequence_file_list = [
            self.check_sequence_name_with_all_version(raw_data_path / sequence_file)
            for sequence_file in self.sample_sequence_list
        ...
        with multiprocessing.Pool(num_workers) as p:
            sequence_infos = list(tqdm(p.imap(produce_sync_lidar_files, sample_sequence_file_list),
                                       total=len(sample_sequence_file_list)))
    ...
    def create_groundtruth_database(self, info_path, save_path, used_classes=None, split='train', sampled_interval=10,
        ...
        point_offset_cnt = 0
        stacked_gt_points = []
        for k in range(0, len(infos), sampled_interval): # len(infos)=158081
...
def create_sync_lidar_points(dataset_cfg, class_names, data_path, save_path,
                       raw_data_tag='raw_data', processed_data_tag='waymo_processed_data',
                       workers=min(16, multiprocessing.cpu_count())):
    dataset = WaymoDataset(
        dataset_cfg=dataset_cfg, class_names=class_names, root_path=data_path,
    ...
    #create synced lidar data, yu hong
    dataset.get_sync_lidar(
        raw_data_path=data_path / raw_data_tag,
        save_path=save_path / processed_data_tag, num_workers=workers,
        sampled_interval=1
    )
