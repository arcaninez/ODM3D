...
class AnchorHeadSingleQFL(AnchorHeadTemplate):
    ...
    def get_cls_layer_loss(self):
        ...
        batch_cls_preds, batch_box_preds = self.generate_predicted_boxes( # decode
            batch_size=batch_size, cls_preds=cls_preds.detach(),
            box_preds=box_preds.detach(), dir_cls_preds=dir_cls_preds.detach()
        )
        iou_targets = None
        cls_targets = None

        for i in range(batch_size):
                iou_3d = iou3d_nms_utils.boxes_iou3d_gpu(batch_box_preds[i], gt_boxes[i][...,:7])
                iou_3d, indice = torch.max(iou_3d, dim=1)

                iou_target = iou_3d.unsqueeze(dim=0)
                if iou_targets == None:
                    iou_targets = iou_target
                else:
                    iou_targets = torch.cat((iou_targets,iou_target), dim=0)

                cls_target = gt_boxes[i][indice][...,-1].unsqueeze(0)
                if cls_targets == None:
                    cls_targets = cls_target
                else:
                    cls_targets = torch.cat((cls_targets,cls_target), dim=0)
    ...
    def forward(self, data_dict):
        ...
        if self.conv_dir_cls is not None:
            ...
            self.forward_ret_dict['dir_cls_preds'] = dir_cls_preds # dir logits
        else:
            dir_cls_preds = None
