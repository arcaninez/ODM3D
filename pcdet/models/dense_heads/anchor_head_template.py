...
class AnchorHeadTemplate(nn.Module):
    ...
    def get_cls_layer_loss(self):
        ...
        if self.num_class == 1: # class agnostic
            box_cls_labels[positives] = 1

        pos_normalizer = positives.sum(1, keepdim=True).float() # number of positive anchors, batch-wise
    ...
    def generate_predicted_boxes(self, batch_size, cls_preds, box_preds, dir_cls_preds=None):
        ...
        Args:
            batch_size:
            cls_preds: (N, H, W, C1); (B,H,W,18)
            box_preds: (N, H, W, C2); (B,H,W,42)
            dir_cls_preds: (N, H, W, C3); (B,H,W,2)

        Returns:
            batch_cls_preds: (B, num_boxes, num_classes); (B,H*W*6,3)
            batch_box_preds: (B, num_boxes, 7+C); (B,H*W*6,7)
