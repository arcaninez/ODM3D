...
class AnchorHeadSingleODM3D(AnchorHeadTemplate):
    ...
    def forward(self, data_dict):
        if self.training:
            self.forward_ret_dict['bev_mask'] = data_dict['bev_mask']
    ...
    def get_cls_layer_loss(self):
        ...
        occupancy_mask = self.forward_ret_dict.get('bev_mask', None)
        normaliser = occupancy_mask.numel() / occupancy_mask.sum()
        if occupancy_mask is not None:
             occupancy_mask = occupancy_mask.view(batch_size,self.forward_ret_dict['cls_preds'].shape[1] * self.forward_ret_dict['cls_preds'].shape[2])
             occupancy_mask = occupancy_mask.repeat(1,6)
             cls_loss = cls_loss * occupancy_mask.unsqueeze(-1)
        cls_loss = cls_loss.sum() / batch_size
        cls_loss = cls_loss * normaliser
    ...
    def get_box_reg_layer_loss(self):
        ...
        box_teacher_pred = box_teacher_pred.view(batch_size, -1, box_teacher_pred.shape[-1] // self.num_anchors_per_location if not self.use_multihead else box_teacher_pred.shape[-1])
        ...
        if occupancy_mask is not None:
             occupancy_mask = occupancy_mask.view(batch_size,self.forward_ret_dict['cls_preds'].shape[1] * self.forward_ret_dict['cls_preds'].shape[2])
             occupancy_mask = occupancy_mask.repeat(1,6)
             loc_loss_src = loc_loss_src * occupancy_mask.unsqueeze(-1)
        loc_loss = loc_loss_src.sum() / batch_size
        loc_loss = loc_loss * normaliser
