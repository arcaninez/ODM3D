...
class ODM3D(nn.Module):
    ...
    def get_training_loss(self, batch_dict):
        ...
        occupancy_mask = batch_dict.get('bev_mask', None)
        ...
        if (bev_img is not None) and (bev_lidar is not None) and self.calculate_bev_loss:
            ...
            normaliser = bev_loss_mask.numel() / bev_loss_mask.sum()

            if self.bev_loss_type == 'L2':
                loss_bev = (self.bev_loss_fun(bev_img,bev_lidar) * bev_loss_mask).mean() * normaliser
...
class ODM3D_MONO(Detector3DTemplate_CMKD):
    ...
    def forward(self, batch_dict):
        ...
        if self.training:
            ...
            for batch_idx in range(batch_size):
                ...
                spatial_occupancy_mask[batch_idx, :, grid_coords[batch_mask, 0], grid_coords[batch_mask, 1]] = 1
            batch_dict['bev_mask'] = spatial_occupancy_mask
...
class ODM3D_LIDAR(Detector3DTemplate_CMKD):
    def __init__(self, model_cfg, num_class, dataset):
        super().__init__(model_cfg=model_cfg, num_class=num_class, dataset=dataset)
        self.module_list = self.build_networks()
        self.bev_layer = model_cfg.get('bev_layer', 'spatial_features')
