...
class CMKD(nn.Module):
    ...
    def get_training_loss(self, batch_dict):
        ...
        if (bev_img is not None) and (bev_lidar is not None) and self.calculate_bev_loss:
            ...
            bev_loss_mask = torch.ones((bev_lidar.shape[0], 1, bev_lidar.shape[2], bev_lidar.shape[3]),
            ...
            if self.bev_loss_type == 'L2':
                ...
                loss_bev = (self.bev_loss_fun(bev_img.sum(dim=1) / C / mean_img,
                                              bev_lidar.sum(dim=1) / C / mean_lidar) * bev_loss_mask).mean()
    ...
    def post_processing(self, batch_dict):
        ...
        for index in range(batch_size):
            ...
            if getattr(self, 'vis_online', False):
                ...
                vis_utils.draw_box_on_img(final_boxes, img, batch_dict['trans_lidar_to_cam'][index],
                                          batch_dict['trans_cam_to_img'][index])
