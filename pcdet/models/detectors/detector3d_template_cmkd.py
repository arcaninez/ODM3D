...
class Detector3DTemplate_CMKD(nn.Module):
    def __init__(self, model_cfg, num_class, dataset):
        ...
        self.module_topology = [
            'vfe', 'backbone_3d', 'map_to_bev_module', 'domain_adaptation', 'pfe',
    ...
    def build_domain_adaptation(self, model_info_dict):

        if self.model_cfg.get('DOMAIN_ADAPTATION', None) is None:
            return None, model_info_dict

        domain_adaption_module = domain_adaptation.__all__[self.model_cfg.DOMAIN_ADAPTATION.NAME](
            model_cfg=self.model_cfg.DOMAIN_ADAPTATION,
            input_channels=model_info_dict['num_bev_features']
        )
        model_info_dict['module_list'].append(domain_adaption_module)
        model_info_dict['num_bev_features'] = domain_adaption_module.num_bev_features
        return domain_adaption_module, model_info_dict
    ...
    def generate_recall_record(box_preds, recall_dict, batch_index, data_dict=None, thresh_list=None):
        ...
        # count from last to find the nonezeros
        # k = cur_gt.__len__() - 1
